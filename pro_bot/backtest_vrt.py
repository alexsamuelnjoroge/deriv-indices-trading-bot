"""
Volatility Regime Transition (VRT) -- proprietary strategy.

Core thesis:
  Markets cycle between low-volatility (compression) and high-volatility (expansion)
  states. The MOMENT of transition from low to high — when ATR crosses above its
  own smoothed average — is the most important signal in this cycle.

  At the transition moment, the direction of the first high-volatility bar is
  non-random: it represents the genuine directional resolution of the compression.
  Smart money has been accumulating during the quiet period; the transition bar
  reveals which direction they chose.

  This is fundamentally different from FCB (which requires sustained multi-timeframe
  compression plus a price breakout). VRT fires at the exact ATR crossover moment,
  which is more surgical and produces more signals.

Signal (1H bars):
  1. Compute ATR(14) and its N-bar Simple Moving Average
  2. Count consecutive bars where ATR < ATR_SMA (quiet state)
  3. Trigger: current bar ATR > ATR_SMA AND previous bar ATR ≤ ATR_SMA
     AND the quiet state lasted at least min_quiet_bars
  4. Body direction: current bar close vs open sets the direction
  5. Body size filter: body must be > min_body × ATR (no doji transitions)
  6. Enter at close, SL: ATR × mult, TP: RR × SL
  7. Cooldown: block re-entry for cooldown_bars after signal

Why ATR SMA instead of ATR of ATR?
  SMA of ATR gives a clean "baseline volatility" for the recent N bars.
  When current ATR exceeds the baseline, the current bar is a volatility event
  relative to recent history — exactly the signal we want.

  Higher atr_sma_period = longer baseline = rarer but more significant transitions.
"""
...
def _atr_sma(atr_series, period):
    """Simple moving average of the ATR series."""
    n   = len(atr_series)
    out = [None] * n
...
def run_vrt(b1h, b1d, cfg):
    atr_sma_period  = cfg["atr_sma_period"]   # baseline smoothing period
    min_quiet_bars  = cfg["min_quiet_bars"]    # minimum consecutive quiet bars before trigger
    min_body_ratio  = cfg["min_body_ratio"]    # body / ATR must exceed this (no doji transitions)
    ...
    signals       = []
    last_sig_i    = -999
    quiet_count   = 0   # consecutive bars in low-vol state
    ...
    for i in range(start, len(b1h)):
        ...
        # Transition trigger: previous bar was quiet, current bar is NOT
        if fa <= sm:
            continue  # still quiet
        if fa_prev > sm_prev:
            continue  # already was elevated — not a fresh transition
        if quiet_count < min_quiet_bars:
            continue  # quiet state was too brief

        # Cooldown
        if i - last_sig_i < cooldown_bars:
            continue

        # Body size filter — the transition bar must show clear direction
        ...
        if bar_bull and allow_long:
            signals.append((i, Signal("BUY", sl_pips=sl, tp_pips=tp,
                                      reason=f"VRT_bull_q{quiet_count}")))
            last_sig_i = i
            quiet_count = 0
        elif not bar_bull and allow_short:
            signals.append((i, Signal("SELL", sl_pips=sl, tp_pips=tp,
                                      reason=f"VRT_bear_q{quiet_count}")))
            last_sig_i = i
            quiet_count = 0
                    ...
                    for tp_rr in [1.5, 2.0, 3.0]:
                        for atr_mult in [1.0, 1.5, 2.0]:
                            for macro in [False, True]:
                                ...
                                sigs = run_vrt(train_b1h, b1d, cfg)
                                if not sigs:
                                    continue
...
async def main():
    ...
    print("=" * 78)
    print("Volatility Regime Transition (VRT) -- 1H -- 4-window walk-forward")
    print(f"BE@1R  max_hold=48H  {DAYS}-day dataset")
    print("Thesis: first bar of vol expansion after sustained quiet = institutional direction signal")
    print("=" * 78 + "\n")
    ...
    for sym, spread in SYMBOLS_TO_TEST:
        ...
        if not ranked:
            print(f"  No profitable VRT configs found for {sym}.\n")
            continue
        ...
        if robust:
            ...
            print(f"  No VRT config passed 3+ windows for {sym}.")
    ...
    if all_robust:
        ...
        print("  No VRT strategy passed 3+ windows on any symbol.")
    print("=" * 78)
