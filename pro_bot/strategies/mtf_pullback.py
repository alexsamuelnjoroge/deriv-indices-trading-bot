...
class MTFPullbackStrategy(BaseProStrategy):
    ...
    def _evaluate(self) -> Signal:
        ...
        # Trend direction by price position, not EMA slope.
        # Slope lags — price crosses EMA days before slope flips, causing wrong-side entries.
        # Price above EMA = uptrend (buy-only); price below EMA = downtrend (sell-only).
        price_now  = htf_closes[-1]
        trend_up   = price_now > ema_now
        trend_down = price_now < ema_now
