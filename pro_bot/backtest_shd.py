"""
Stop Hunt Detection (SHD) -- proprietary strategy.

Core thesis:
  Institutions deliberately push price beyond structural highs/lows to trigger
  stop orders clustered there, then reverse. They use retail stop-loss momentum
  to fill their own large orders at better prices.

  Fingerprint: the "hunt" bar violates the N-bar structural level with its WICK
  but its CLOSE reverses back inside. The wick is the hunt; the close is the trap
  springing shut.

  This is structurally different from CRD (anomalous candle SIZE).
  SHD requires a SPECIFIC LEVEL VIOLATION that reverses — the fingerprint of
  stop hunting, not just a large candle.

Signal (1H bars):
  1. Track rolling N-bar highest high and lowest low (excluding current bar)
  2. SELL trigger:
       bar.high > n_bar_high  (wick penetrated the structural level)
       bar.close < n_bar_high (close reversed back inside)
       (bar.high - n_bar_high) >= min_probe * ATR  (probe large enough to hit stops)
  3. BUY trigger (mirror):
       bar.low < n_bar_low
       bar.close > n_bar_low
       (n_bar_low - bar.low) >= min_probe * ATR
  4. SL: ATR * mult, TP: RR * SL
  5. Cooldown, macro filter (daily EMA)

Why the minimum probe?
  A 1-pip breach may not trigger real stop orders. The wick must penetrate far
  enough to realistically sweep clustered stops. min_probe * ATR ensures it.
"""
...
def run_shd(b1h, b1d, cfg):
    n_bars        = cfg["n_bars"]
    min_probe     = cfg["min_probe"]       # ATR multiples the wick must exceed the level
    ...
    for i in range(start, len(b1h)):
        ...
        # SELL hunt: wick spiked above structural high, close reversed inside
        probe_up = bar["high"] - struct_high
        if (probe_up >= min_probe * atr_val
                and bar["close"] < struct_high
                and allow_short):
            signals.append((i, Signal("SELL", sl_pips=sl, tp_pips=tp,
                                      reason=f"SHD_sell_probe{probe_up:.4f}_n{n_bars}")))
            last_sig_i = i
            continue

        # BUY hunt: wick spiked below structural low, close reversed inside
        probe_dn = struct_low - bar["low"]
        if (probe_dn >= min_probe * atr_val
                and bar["close"] > struct_low
                and allow_long):
            signals.append((i, Signal("BUY", sl_pips=sl, tp_pips=tp,
                                      reason=f"SHD_buy_probe{probe_dn:.4f}_n{n_bars}")))
            last_sig_i = i
...
def sweep(b1h, b1d, train_end_idx, spread):
    ...
    for n_bars in [10, 20, 30]:
        for min_probe in [0.1, 0.3, 0.5]:
                ...
                for tp_rr in [1.5, 2.0, 3.0]:
                    for atr_mult in [1.0, 1.5, 2.0]:
                        for macro in [False, True]:
                            ...
                            label = (
                                f"SHD n{n_bars} probe>{min_probe}ATR "
                                f"cool{cooldown} RR{tp_rr} ATRx{atr_mult} "
                                f"{'MACRO' if macro else 'free'}"
                            )
                            sigs = run_shd(train_b1h, b1d, cfg)
                            if not sigs:
                                continue
...
async def main():
    ...
    print("=" * 78)
    print("Stop Hunt Detection (SHD) -- 1H -- 4-window walk-forward")
    print(f"BE@1R  max_hold=48H  {DAYS}-day dataset")
    print("Thesis: wick-through structural level + close inside = institutional stop hunt")
    print("Fade the hunt: institutions ran stops to fill their own position — reverse follows")
    print("=" * 78 + "\n")
    ...
    for sym, spread in SYMBOLS_TO_TEST:
        ...
        if not ranked:
            print(f"  No profitable SHD configs found for {sym}.\n")
            continue
        ...
        if robust:
            ...
            print(f"  No SHD config passed 3+ windows for {sym}.")
    ...
    if all_robust:
        ...
        print("  No SHD strategy passed 3+ windows on any symbol.")
    print("=" * 78)
